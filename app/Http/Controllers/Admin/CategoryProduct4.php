...
class CategoryProduct4 extends Controller
{
  ...
  public function __construct()
  {
    ...
    $this->uploadCategoryProduct = "public/upload/category_product4";
  ...
  public function index(Request $request)
  {
    ...
    $row3 = CategoryProduct::where('type', $type)->where('level', 3)->orderBy('num', 'ASC')->orderBy('id', 'ASC')->get();
    $keyword = htmlspecialchars($request->input('keyword'));
    if ($keyword) {
      $rows = CategoryProduct::where("title", "LIKE", "%{$keyword}%")->where('type', $this->typeCategory4)->where('level', 4)->orderBy('num', 'ASC')->orderBy('id', 'ASC')->paginate($this->numberPerPage)->appends($categoryAppendQueryString);
    } else {
      if ($request->category) {
        $rows = CategoryProduct::where('type', $this->typeCategory4)->orWhere('id_parent', $request->category)->where('level', 4)->orderBy('num', 'ASC')->orderBy('id', 'ASC')->paginate($this->numberPerPage)->appends($categoryAppendQueryString);
      } else {
        $rows = CategoryProduct::where('type', $this->typeCategory4)->where('level', 4)->orderBy('num', 'ASC')->orderBy('id', 'ASC')->paginate($this->numberPerPage)->appends($categoryAppendQueryString);
      }
    }
    return view('admin.category_product_level4.index', compact('rows', 'row3', 'type', 'level'));
  }
  ...
  public function create()
  {
    ...
    $level = '4';
    session(['module_active' => $type . '-4']);
    $rowCategory1 = CategoryProduct::where('type', $this->typeCategory4)->where('level', 3)->orderBy('num', 'ASC')->orderBy('id', 'ASC')->get();
    return view('admin.category_product_level4.create', compact('rowCategory1', 'type', 'level'));
  }
  ...
  public function save(Request $request)
  {
    ...
    if (!$validator->fails()) {
      ...
      if ($request->hasFile('photo1')) {
        ...
        $path = public_path('upload/category_product4');
  ...
  public function show(Request $request)
  {
    $type = $this->type;
    $level = '4';
    $row = CategoryProduct::where('type', $this->typeCategory4)->find($request->id);
    $rowSeo = Seo::where('type', $this->typeCategory4)->where('hash_seo', $row->hash)->first();
    $rowCategory1 = CategoryProduct::where('type', $this->typeCategory3)->where('level', 3)->orderBy('num', 'ASC')->orderBy('id', 'ASC')->get();
    return view('admin.category_product_level4.show', compact('row', 'rowSeo', 'rowCategory1', 'type', 'level'));
  }

  /* Category product update */
  public function update(Request $request)
  {
    $categoryProduct = CategoryProduct::where('type', $this->typeCategory4)->find($request->id);
    if (!file_exists($this->uploadCategoryProduct)) {
      mkdir($this->uploadCategoryProduct, 0777, true);
    }
    $validator = Validator::make(
    ...
    if (!$validator->fails()) {
      ...
      return redirect()->route('admin.category_product4.show', ['id' => $categoryProduct->id])->withErrors($validator)->withInput();
    }
  }
  ...
  {
    $row = CategoryProduct::where('type', $this->typeCategory4)->find($id);
  ...
  public function delete($id, $hash)
  {
    ...
    $categoryProduct = CategoryProduct::where('type', $this->typeCategory4)->where('hash', $hash)->find($id);
  ...
  {
    $upload = "public/upload/category_product4/";
    $categoryProduct = CategoryProduct::where('type', $this->typeCategory4)->find($request->checkitem);
  ...
  {
    CategoryProduct::where('id', $request->id)->where('type', $this->typeCategory4)->update(['num' => $request->value]);
  }

  /* Category product update status ajax */
  public function updateStatus(Request $request)
  {
    $status = CategoryProduct::select('status')->where('type', $this->typeCategory4)->find($request->id)->status;
    ...
    CategoryProduct::where('id', $request->id)->where('type', $this->typeCategory4)->update(['status' => $statusStr]);
  }
  ...
  public function deletePhoto(Request $request)
  {
    ...
    $photo = CategoryProduct::where('type', $this->typeCategory4)->find($request->id)->$action;
    $photo = isset($photo) && !empty($photo) ? $photo : "";
    if (file_exists($upload . $photo) && !empty($photo)) unlink($upload . $photo);
    CategoryProduct::where('id', $request->id)->where('type', $this->typeCategory4)->update([$action => null]);
