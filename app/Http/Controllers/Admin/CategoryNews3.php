...
class CategoryNews3 extends Controller
{
  ...
  public function index(Request $request)
  {
    ...
    $row2 = CategoryNews::where('type', $type)->where('level', 2)->orderBy('num', 'ASC')->orderBy('id', 'ASC')->get();
    $keyword = htmlspecialchars($request->input('keyword'));
    if ($keyword) {
      $rows = CategoryNews::where("title", "LIKE", "%{$keyword}%")->where('type', $this->typeCategory3)->where('level', 3)->orderBy('num', 'ASC')->orderBy('id', 'ASC')->paginate($this->numberPerPage)->appends($categoryAppendQueryString);
    } else {
      if ($request->category) {
        $rows = CategoryNews::where('type', $this->typeCategory3)->where('id_parent', $request->category)->where('level', 3)->orderBy('num', 'ASC')->orderBy('id', 'ASC')->paginate($this->numberPerPage)->appends($categoryAppendQueryString);
      } else {
        $rows = CategoryNews::where('type', $this->typeCategory3)->where('level', 3)->orderBy('num', 'ASC')->orderBy('id', 'ASC')->paginate($this->numberPerPage)->appends($categoryAppendQueryString);
      }
    }
    return view('admin.category_news_level3.index', compact('rows', 'row2', 'type', 'level'));
  }
  ...
  public function create()
  {
    ...
    return view('admin.category_news_level3.create', compact('rowCategory1', 'type', 'level'));
  }
  ...
  public function show(Request $request)
  {
    ...
    return view('admin.category_news_level3.show', compact('row', 'rowSeo', 'rowCategory1', 'type', 'level'));
  }
  ...
  public function update(Request $request)
  {
    ...
    if (!$validator->fails()) {
      ...
      return redirect()->route('admin.category_news3.show', ['id' => $categoryNews->id])->withErrors($validator)->withInput();
    }
  }
