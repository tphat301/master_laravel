...
class CategoryNews4 extends Controller
{
  protected $helper;
  ...
  public function __construct()
  {
    ...
    $this->numberPerPage = config('admin.' . $this->type . '.category.category4.number_per_page');
    $this->uploadCategoryNews = "public/upload/category_news4";
    $this->width1 = config('admin.' . $this->type . '.category.category4.width1');
    $this->width2 = config('admin.' . $this->type . '.category.category4.width2');
    $this->width3 = config('admin.' . $this->type . '.category.category4.width3');
    $this->width4 = config('admin.' . $this->type . '.category.category4.width4');
    $this->height1 = config('admin.' . $this->type . '.category.category4.height1');
    $this->height2 = config('admin.' . $this->type . '.category.category4.height2');
    $this->height3 = config('admin.' . $this->type . '.category.category4.height3');
    $this->height4 = config('admin.' . $this->type . '.category.category4.height4');
  }
  ...
  public function index(Request $request)
  {
    $type = $this->type;
    $level = '4';
    session(['module_active' => $type . '-4']);
    $categoryAppendQueryString = ['category' => $request->category];
    $row3 = CategoryNews::where('type', $type)->where('level', 3)->orderBy('num', 'ASC')->orderBy('id', 'ASC')->get();
    $keyword = htmlspecialchars($request->input('keyword'));
    if ($keyword) {
      $rows = CategoryNews::where("title", "LIKE", "%{$keyword}%")->where('type', $this->typeCategory4)->where('level', 4)->orderBy('num', 'ASC')->orderBy('id', 'ASC')->paginate($this->numberPerPage)->appends($categoryAppendQueryString);
    } else {
      if ($request->category) {
        $rows = CategoryNews::where('type', $this->typeCategory4)->where('id_parent', $request->category)->where('level', 4)->orderBy('num', 'ASC')->orderBy('id', 'ASC')->paginate($this->numberPerPage)->appends($categoryAppendQueryString);
      } else {
        $rows = CategoryNews::where('type', $this->typeCategory4)->where('level', 4)->orderBy('num', 'ASC')->orderBy('id', 'ASC')->paginate($this->numberPerPage)->appends($categoryAppendQueryString);
      }
    }
    return view('admin.category_news_level4.index', compact('rows', 'row3', 'type', 'level'));
  }
  ...
  public function create()
  {
    ...
    $level = '4';
    session(['module_active' => $type . '-4']);
    $rowCategory1 = CategoryNews::where('type', $this->typeCategory4)->where('level', 3)->orderBy('num', 'ASC')->orderBy('id', 'ASC')->get();
    return view('admin.category_news_level4.create', compact('rowCategory1', 'type', 'level'));
  }
  ...
  public function save(Request $request)
  {
    ...
    if (!$validator->fails()) {
      ...
      if ($request->hasFile('photo1')) {
        ...
        $path = public_path('upload/category_news4');
      ...
      return $this->helper->transfer("Thêm dữ liệu", "success", route('admin.category_news4'));
    } else {
      return redirect()->route('admin.category_news4.create')->withErrors($validator)->withInput();
    }
  }
  ...
  public function show(Request $request)
  {
    $type = $this->type;
    $level = '4';
    $row = CategoryNews::where('type', $this->typeCategory4)->find($request->id);
    $rowSeo = Seo::where('type', $this->typeCategory4)->where('hash_seo', $row->hash)->first();
    $rowCategory1 = CategoryNews::where('type', $this->typeCategory3)->where('level', 3)->orderBy('num', 'ASC')->orderBy('id', 'ASC')->get();
    return view('admin.category_news_level4.show', compact('row', 'rowSeo', 'rowCategory1', 'type', 'level'));
  }

  /* Category news update */
  public function update(Request $request)
  {
    $categoryNews = CategoryNews::where('type', $this->typeCategory4)->find($request->id);
    if (!file_exists($this->uploadCategoryNews)) {
      mkdir($this->uploadCategoryNews, 0777, true);
    }
    $validator = Validator::make(
    ...
    if (!$validator->fails()) {
      ...
      return $this->helper->transfer("Cập nhật dữ liệu", "success", route('admin.category_news4.show', ['id' => $categoryNews->id]));
    } else {
      return redirect()->route('admin.category_news4.show', ['id' => $categoryNews->id])->withErrors($validator)->withInput();
    }
  }
  ...
  {
    $row = CategoryNews::where('type', $this->typeCategory4)->find($id);
  ...
  {
    $upload = "public/upload/category_news4/";
    $categoryNews = CategoryNews::where('type', $this->typeCategory4)->where('hash', $hash)->find($id);
    $seo = SEO::where('type', $this->typeCategory4)->where('hash_seo', $hash);
    ...
    $categoryNews->delete();
    $seo->delete();
    return $this->helper->transfer("Xóa dữ liệu", "success", route('admin.category_news4'));
  }
  ...
  {
    $upload = "public/upload/category_news4/";
    $categoryNews = CategoryNews::where('type', $this->typeCategory4)->find($request->checkitem);
    ...
    Seo::where('type', $this->typeCategory4)->whereIn('hash_seo', $request->hashes)->delete();
  ...
  {
    CategoryNews::where('id', $request->id)->where('type', $this->typeCategory4)->update(['num' => $request->value]);
  }

  /* Category news update status ajax */
  public function updateStatus(Request $request)
  {
    $status = CategoryNews::select('status')->where('type', $this->typeCategory4)->find($request->id)->status;
    ...
    CategoryNews::where('id', $request->id)->where('type', $this->typeCategory4)->update(['status' => $statusStr]);
  }
  ...
  public function deletePhoto(Request $request)
  {
    ...
    $photo = CategoryNews::where('type', $this->typeCategory4)->find($request->id)->$action;
    $photo = isset($photo) && !empty($photo) ? $photo : "";
    if (file_exists($upload . $photo) && !empty($photo)) unlink($upload . $photo);
    CategoryNews::where('id', $request->id)->where('type', $this->typeCategory4)->update([$action => null]);
    return $this->helper->transfer("Xóa dữ liệu", "success", route('admin.category_news4.show', ['id' => $request->id]));
  }
}
