  ...
  <section class="content">
    {!! Form::open(['name' => 'form-news-list', 'class' => ['form-product-list'], 'method' => 'GET']) !!}
      <div class="card-footer text-sm sticky-top">
        <a class="btn btn-sm bg-gradient-primary text-white" href="{{route('admin.category_news'.$type.'.create')}}" title="Thêm mới"><i class="fas fa-plus mr-2"></i>Thêm mới</a>

        <a data-url="{{ route('admin.category_news'.$type.'.destroy') }}" class="btn btn-sm bg-gradient-danger text-white delete-all" id="delete-all"><i class="far fa-trash-alt mr-2"></i>Xóa tất cả</a>
              ...
              @if ($rows->total() > 0)
                @foreach ($rows as $k => $row)
                  <tr>
                    ...
                    <td class="align-middle">
                      <input type="number" class="update-num form-control form-control-mini m-auto" min="0" value="{{ $row->num }}" data-id="{{ $row->id }}" data-url="{{ route('admin.category_news'.$type.'.update_number') }}"/>
                    </td>
                    <td class="align-middle">
                      <a href="{{ route('admin.category_news'.$type.'.show', [$row->id]) }}" title="{{ $row->title }}">
                        @if (!empty($row->photo1))
                    ...
                    <td class="align-middle">
                      <a class="text-dark text-break" href="{{ route('admin.category_news'.$type.'.show', [$row->id]) }}" title="{{ $row->title }}">
                        {{ $row->title }}
                      </a>
                    </td>
                    @foreach (config('admin.'.$type.'.category.category'.$level.'.status') as $key => $value)
                      <td class="align-middle text-center">
                        <div class="custom-control custom-checkbox">
                          ...
                          @endphp
                          <input type="checkbox" id="update-status-{{$key}}-{{$k}}" class="update-status custom-control-input" name="{{ $key }}" data-id="{{ $row->id }}" data-url="{{route('admin.category_news'.$type.'.update_status')}}" {{ in_array($key, $status) ? 'checked' : '' }} />
                          <label for="update-status-{{$key}}-{{$k}}" class="custom-control-label"></label>
                        </div>
                      </td>
                    @endforeach
                    <td class="align-middle text-center text-md text-nowrap">
                      <a class="text-primary mr-2" href="{{ route('admin.category_news'.$type.'.show', [$row->id]) }}" title="Chỉnh sửa">
                        <i class="fas fa-edit"></i>
                      </a>

                      @if(config('admin.'.$type.'.category.category'.$level.'.copy') === true)
                        <div class="dropdown d-inline-block align-middle">
                          <a href="{{ route('admin.category_news'.$type.'.copy', [$row->id]) }}" class="nav-link text-success p-0 pr-2" title="Copy">
                            <i class="far fa-clone"></i>
                          </a>
                        </div>
                      @endif

                      <a class="text-danger delete-row" data-url="{{route('admin.category_news'.$type.'.delete', ['id' => $row->id, 'hash' => $row->hash])}}" title="Xóa" style="cursor: pointer">
