            ...
            <div class="form-row">
              @foreach ($rowSame as $k => $v)
                ...
                <div class="news-other d-flex flex-wrap col-12 col-lg-12 col-md-6">
                  <a class="scale-img text-decoration-none pic-news-other" href="{{route('policy',['slug'=>$nameId])}}" title="{{$v->title}}">
                    <img loading="lazy" class="w-100" src="{{!empty($v->photo1) ? asset('upload/post/'.$v->photo1) : url('resources/images/noimage.png')}}" alt="{{ !empty($v->photo1) ? $v->title : 'Noimage' }}" title="{{ !empty($v->photo1) ? $v->title : 'Noimage' }}" />
                  </a>
                  <div class="info-news-other">
                    <h3>
                      <a class="name-news-other text-decoration-none text-split" href="{{route('policy',['slug'=>$nameId])}}" title="{{$v->title}}">
                        {{$v->title}}
                      </a>
                    </h3>
                  </div>
